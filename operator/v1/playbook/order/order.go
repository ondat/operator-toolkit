...
func StepRequeueStrategy(step []operand.Operand) operand.RequeueStrategy {
	strategy := operand.RequeueOnError
	for _, o := range step {
		if o.RequeueStrategy() == operand.RequeueAlways {
			strategy = operand.RequeueAlways
			break
		}
	}
	return strategy
